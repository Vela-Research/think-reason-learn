...
class WeightTrainerConfig:
    ...
    positive_weights : bool
        If True, set negative coefficients to zero.
    normalize : bool
...
class PolicyInduction:
    ...
    def _validate_config(self, cfg) -> WeightTrainerConfig:
        """Validate or create a WeightTrainerConfig object."""
        if cfg is None:
            return WeightTrainerConfig()
        elif isinstance(cfg, WeightTrainerConfig):
            return cfg
        else:
            raise ValueError(
                f"config must be a WeightTrainerConfig instance or None, "
    ...
    def _check_memory(self) -> None:
        ...
        required = {"predictions", "policy", "weight"}
        ...
        is_predictions_series = pm["predictions"].apply(
            lambda x: isinstance(x, pd.Series)
        )

        before_rows = len(pm)
        pm = pm[is_policy_str & is_predictions_series].copy()
        after_rows = len(pm)
        removed = before_rows - after_rows

        logger.info(
            f"_check_memory: removed {removed} invalid rows "
            f"(before={before_rows}, after={after_rows})"
        )

        if after_rows == 0:
            raise ValueError("Invalid _policy_memory: empty after type filtering")
    ...
    def _build_feature_matrix(
        ...
        for idx, row in pm.iterrows():
            ...
            col_name = str(idx)
            X_df[col_name] = mapped.reindex(sample_index).astype(float)
        ...
        return X_df.values, y_num, sample_index, np.array(col_names, dtype=str)  # type: ignore
    ...
    def _fit_weights(self) -> None:
        ...
        logger.info(
            f"Fitting weights: {n_samples} samples × {n_models} models, "
            f"config={asdict(cfg)}"
        )
        ...
        final_lr.fit(X, y)
        coef_raw = final_lr.coef_.ravel()
        coef_proc = (
            np.where(coef_raw < 0, 0.0, coef_raw) if cfg.positive_weights else coef_raw
        )
        if cfg.normalize:
            s = np.sum(np.abs(coef_proc))
            coef_proc = (
                np.ones_like(coef_proc) / len(coef_proc) if s == 0 else coef_proc / s
            )

        weights = pd.Series(coef_proc, index=col_names)
        pm_index_str = [str(i) for i in self._policy_memory.index]
        weights = weights.reindex(pm_index_str).fillna(0.0)
        self._policy_memory["weight"] = weights.values
        ...
        result = {
            ...
            "beta": cfg.beta,
            "coef_raw": pd.Series(coef_raw, index=col_names),
            "coef_processed": pd.Series(coef_proc, index=col_names),
