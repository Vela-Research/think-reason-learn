...
class RRF:
    ...
    def __init__(
        ...
        assert all(cr > 0 for cr in class_ratio) and len(class_ratio) == 2, (
            "class_ratio must have exactly two elements each > 0"
        )
    ...
    async def _generate_questions(self) -> None:
        ...
        for sample in self._sample(self.max_samples_as_context):
            samples_str = sample[self._X_column].to_string(index=False)  # type: ignore
    ...
    def _question_excluded(self, qid: str) -> bool:
        try:
            ...
            # These columns do not exist, it's a production model
            return False
    ...
    def save(
        ...
        if not for_production:
            ...
            qdf = self._questions[  # type: ignore
                ~self._questions["expert_excluded"]
                & ~self._questions["excluded_in_semantics"]
                & ~self._questions["excluded_in_pred_similarity"]
            ].copy()
            qdf = cast(pd.DataFrame, qdf)
            qdf = qdf.drop(
                columns=[
                    "embedding",
                    "expert_excluded",
        ...
        manifest = {
            ...
            "qanswer_llmc": _serialize_llm_priorities(self.qanswer_llmc),
